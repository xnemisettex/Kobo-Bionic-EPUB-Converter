...
def convert(source, destination):
    ...
    try:
        ...
        with zipfile.ZipFile(source) as zin:
            ...
            new_opf_text = opf_text[:title_match.start(2)] + title_match.group(2) + ' (Bionic Strong)' + opf_text[title_match.end(2):]
...
def output_path(source):
    ...
    if name.casefold().endswith('.kepub.epub'):
        return source.with_name(name[:-len('.kepub.epub')] + ' (Bionic Strong).kepub.epub')
    return source.with_name(name[:-len('.epub')] + ' (Bionic Strong).epub')
...
def main():
    ap = argparse.ArgumentParser(description='Convert Kobo books with strong (60%) Bionic formatting.')
